...
the resources are actually created or modified. There are two types of hooks:
...
In order to minimize Kopf's footprint in production systems,
...
For example, Kopf's own size with critical dependencies is 8.8 MB,
...
.. code-block:: python
    ...
    @kopf.on.validation('kopfexamples')
    def say_hello(**_):
        warnings.warn("Verified with the operator's hook.")
    ...
    @kopf.on.validation('kopfexamples')
    def convertible_numbers(spec, **_):
            ...
            for val in spec.get('numbers', []):
                if not isinstance(val, float):
                    try:
                        ...
                        warnings.warn(f"{val!r} is not a number but can be converted.")
...
Each handler is mapped to its own admission webhook and an endpoint,
...
if such logic is needed, make it as one handler with sequential execution.
...
The sematics is the same or as close as possible to the Kubernetes API's one.
``None`` values will remove the relevant keys.

Under the hood, the patch object will remember each individual change
and will return a JSONPatch structure back to Kubernetes.
...
This has no effect if there are no managed configuration.
The webhook cleanup only happens on graceful exits; on forced exits, even
non-persisted webhooks might in fact be persisted and block the admissions.
...
.. warning::
    ...
    the cluster as unusable until the configuration is manually removed.

    Start the development in local clusters, validating/mutating the custom
    resources first, and enable ``ignore_errors`` initially.
    Stricten the handlers only when stabilised.


In-memory containers
====================

Kopf provides :doc:`/memos` for each individual resource. However, webhooks
can happen before a resource is created. This affects how the memos work.

For update and deletion requests, the actual memos of the resources are used.

For creation admission requests, a memo is created and immediately discarded.
It means that the creation's memos are useless at the moment.
...
Only the warnings of category `UserWarning` (and descendants) are intercepted:
...
.. code-block:: python
    ...
    @kopf.on.validation('kopfexamples')
    def ensure_default_numbers(spec, **_):
...
In order to manage the validating/mutating webhook configurations,
Kopf requires the following RBAC permissions in its service account
(see :doc:`/deployment`):
...
To fulfill its promise, Kopf delegates this task to webhook servers and tunnels,
which are capable of receiving the webhook requests, marshalling them
to the handler callbacks, and then returning the results back to Kubernetes.
...
each with its own configuration parameters (see their descriptions):
...
.. note::
    External tunneling services usually limit the number of requests per minute.
    ...
    If that is not an option, you can implement you own free tunneling.
...
When and if needed, the operator developers can implement their own
servers/tunnels with their own customised authentication methods.
...
do not support HTTPS tunneling (or require paid subscriptions):
...
Operator developers can provide their own custom servers and tunnels
by implementing an async iterator over client configs (`kopf.ClientConfig`).
There are two ways to implement servers/tunnels depending on configurability.

One is a simple but non-configurable coroutine:

.. code-block:: python

    async def mytunnel(callback: kopf.AdmissionCallback) -> AsyncIterator[kopf.AdmissionClientConfig]:
        ...
        yield client_config
...
.. code-block:: python
    ...
    class MyTunnel:
        async def __call__(self, callback: kopf.AdmissionCallback) -> AsyncIterator[kopf.AdmissionClientConfig]:
            ...
            yield client_config
            await asyncio.Event().wait()
...
The iterator MUST accept a positional argument of type `kopf.AdmissionCallback`
...
* ``webhook`` (``str``) -- to execute only one specific handler/webhook;
  otherwise, all applicable admission handlers are executed. This id
  usually comes from the URL, which the framework extends automatically.
...
.. code-block:: python
    ...
    response = await callback(request, webhook=handler_id, headers=headers, sslpeer=sslpeer)
...
The iterator must yield one or more client configs. The config is a dictionary
that goes to the managed webhook configurations as ``.webhooks.*.clientConfig``.
...
.. code-block:: python
    ...
    class MyTunnel1(kopf.WebhookServer):
        async def __call__(self, callback: kopf.AdmissionCallback) -> AsyncIterator[kopf.AdmissionClientConfig]:
            ...
            for client_config in super().__call__(callback):
    ...
    class MyTunnel2:
        async def __call__(self, callback: kopf.AdmissionCallback) -> AsyncIterator[kopf.AdmissionClientConfig]:
            ...
            for client_config in server(callback):
                ...  # renew a tunnel, adjust the config
                yield client_config
